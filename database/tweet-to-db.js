...
function saveToDatabaseApiV1(tweet) {
  ...
  if (parseInt(getDateString(tweet.created_at).substring(0, 4)) < 2021) return;
  ...
  db.parallelize(function () {
    ...
    stmt.run(
      tweet.id_str,
      getDateString(tweet.created_at),
...
function saveToDatabase(tweet, users, mediaObjects) {
  ...
  let replies = (tweet.referenced_tweets || []).filter(
    (entry) => entry.type === "replied_to"
  );
  let replyTweetId = replies.length ? replies[0].id : null;

  let userEntry = users.filter(
    (entry) => entry.id === tweet.in_reply_to_user_id
  );
