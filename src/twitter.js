...
class Twitter {
  ...
  async renderFullText(tweet) {
    ...
    if (tweet.entities) {
      for (let url of tweet.entities.urls) {
        if (
          url.expanded_url &&
          url.expanded_url.indexOf(`/${tweet.id}/photo/`) > -1
        ) {
          ...
          if (
            ...
            medias.push(
              `<template data-island><a href="${targetUrl}"><img src="https://v1.opengraph.11ty.dev/${encodeURIComponent(
                targetUrl
              )}/small/onerror/" alt="OpenGraph image for ${displayUrl}" loading="lazy" decoding="async" width="375" height="197" class="tweet-media tweet-media-og" onerror="this.parentNode.remove()"></a></template>`
            );
          }
        }
      }
    ...
    if (medias.length) {
      text += `<is-land on:visible><div class="tweet-medias">${medias.join(
        ""
      )}</div></is-land>`;
    }
    return text;
  ...
  async renderTweet(tweet, options = {}) {
    ...
    return `<li id="${tweet.id_str}" class="tweet${
      ...
      tweet.id_str
    }" class="tag tag-icon"><span class="sr-only">On twitter.com </span><img src="${this.avatarUrl(
      "https://twitter.com/"
